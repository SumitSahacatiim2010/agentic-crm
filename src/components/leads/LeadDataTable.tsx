...
export function LeadDataTable({ initialLeads, onSelectLead }: LeadDataTableProps) {
    const { data: swrData, mutate } = useSWR('/api/leads?limit=100', fetcher, { fallbackData: { data: initialLeads } });
            ...
            <Table>
                ...
                <TableBody>
                    {leads.map((lead: any) => (
                        <TableRow
                            key={lead.id}
                            className={`border-slate-800 hover:bg-indigo-900/10 cursor-pointer transition-all duration-200 ${selectedLeadId === lead.id ? 'bg-indigo-950/40 border-l-4 border-l-indigo-500' : ''}`}
                        ...
                        >
                            ...
                            <TableCell className="px-6 py-4">
                                <Badge variant="outline" className={`text-[10px] font-bold px-2 py-0.5 capitalize ${lead.lead_rating?.toLowerCase() === 'hot' ? 'bg-red-500/10 text-red-400 border-red-500/30' : lead.lead_rating?.toLowerCase() === 'warm' ? 'bg-amber-500/10 text-amber-400 border-amber-500/30' : 'bg-blue-500/10 text-blue-400 border-blue-500/30'}`}>
                            ...
                            <TableCell className="px-6 py-4">
                                <Badge variant="outline" className={`text-[10px] font-bold px-2 py-0.5 capitalize
                                    ...
                                    ${lead.status?.toLowerCase() === 'qualified' ? 'text-indigo-400 border-indigo-500/30' : ''}
                                    ${lead.status?.toLowerCase() === 'converted' ? 'text-emerald-400 border-emerald-500/30' : ''}
                                    ${lead.status?.toLowerCase() === 'lost' ? 'text-slate-500 border-slate-700' : ''}
                                `}>
                                    {lead.status}
                                </Badge>
                            </TableCell>
